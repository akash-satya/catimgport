...
char supportsUTF8() {
    ...
    return (lc_all_env && strstr(lc_all_env, UTF))
        || (lang_env && strstr(lang_env, UTF))
        || (lc_ctype_env && strstr(lc_ctype_env, UTF));
}
    ...
    while (loop++ < loops || loops < 0) {
        ...
        for (uint32_t frame = 0; frame < img.frames; frame++) {
            if (frame > 0 || loop > 0) {
    			...
    			struct timespec ts;
    			ts.tv_sec = 0;
    			ts.tv_nsec = (img.delays[frame - 1] * 10000) * 1000;  // Convert to nanoseconds
    			nanosleep(&ts, NULL);
		}
		else {
    			struct timespec ts;
    			ts.tv_sec = 0;
    			ts.tv_nsec = (img.delays[img.frames - 1] * 10000) * 1000;  // Convert to nanoseconds
   			nanosleep(&ts, NULL);
		}
		printf("\e[u");
            }
